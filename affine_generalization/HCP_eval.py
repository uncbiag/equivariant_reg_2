...
if args.network_name == "rotated":
    import hcp_equicon_train_rotated
    net = hcp_equicon_train_rotated.make_net(dimension=3)
    utils.log(net.regis_net.load_state_dict(torch.load(weights_path), strict=True))
    net.eval()
if args.network_name == "rotated_unrotate":
    import results.compile_26.hcp_equicon_train_rotated
    net = results.compile_26.hcp_equicon_train_rotated.make_net(dimension=3)
    utils.log(net.regis_net.load_state_dict(torch.load(weights_path), strict=True))
    net.eval()
