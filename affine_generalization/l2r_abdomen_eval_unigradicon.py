...
for (fixed_path, moving_path) in tqdm(test_cases[:]):
    ...
    phi_AB, phi_BA = icon.itk_wrapper.register_pair(net, register.preprocess(raw_moving), register.preprocess(raw_fixed), finetune_steps=None)
    import affine_decomposition
    phi_AB = affine_decomposition.decompose_icon_itk_transform(phi_AB)

    cmoving = itk.CastImageFilter[type(raw_moving), itk.Image[itk.F, 3]].New()(raw_moving)
    interpolator = itk.LinearInterpolateImageFunction.New(cmoving)
    warped_moving_image = itk.resample_image_filter(
            cmoving,
            transform=phi_AB,
            interpolator=interpolator,
            use_reference_image=True,
            reference_image=raw_fixed
            )

    ug = unigradicon.get_unigradicon()
    unigradicon_phi_AB, _ = icon.itk_wrapper.register_pair(ug, unigradicon.preprocess(warped_moving_image), unigradicon.preprocess(raw_fixed), finetune_steps=None)


    double_transform = itk.CompositeTransform[itk.D, 3].New()

    double_transform.PrependTransform(unigradicon_phi_AB)
    double_transform.PrependTransform(phi_AB)

    dispfield_filter = itk.TransformToDisplacementFieldFilter[itk.Image[itk.Vector[itk.F, 3], 3], itk.D].New()

    dispfield_filter.SetTransform(double_transform)
