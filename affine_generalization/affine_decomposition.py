...
moving = itk.imread("/home/hastings/Downloads/AbdomenMRCT_0002_0001.nii.gz")
fixed = itk.imread("/home/hastings/Downloads/AbdomenMRCT_0002_0000.nii.gz")

phi_AB = itk.transformread("/home/hastings/Downloads/trans.hdf5")[0]
...
itk.transformwrite([phi_AB], "decomposed.hdf5")
...
def preprocess(image):
    ...
    min_ = quantile(torch.tensor(np.original_displacement_array(image)), .01).item()
    max_ = quantile(torch.tensor(np.original_displacement_array(image)), .99).item()
...
for _ in range(2):
    plt.imshow(np.minimum(100, -np.array(preprocess(warped_moving_image)[:, 50])), cmap="Grays")
    plt.show()
    plt.pause(.9)

    plt.imshow(np.minimum(100, -np.array(preprocess(fixed)[:, 50])), cmap="Grays")
    plt.show()
    plt.pause(.9)

plt.imshow(np.minimum(100, -np.array(itk.checker_board_image_filter(preprocess(warped_moving_image), preprocess(fixed))[:, 50])), cmap="Grays")
