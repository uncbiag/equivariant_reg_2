...
ds, ds2 = icon_registration.data.get_dataset_retina(fixed_vertical_offset=200)
...
ts = icon.network_wrappers.DownsampleNet(ts, 2)
...
net = icon.losses.DiffusionRegularizedNet(ts, icon.LNCC(sigma=4), lmbda=3)
...
curves = icon.train_datasets(net, optim, ds, ds2, epochs=30)
plt.close()
...
#ds, ds2 = icon_registration.data.get_dataset_retina(split="test")
