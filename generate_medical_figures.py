...
def render_pair(name, image_A, phi_AB, image_B, net):
    ...
    def show(im, **kwargs):
        plt.axis('off')
        fig = plt.imshow(im, cmap="gray", aspect=aspect)
...
net = make_net(net_input_shape)
weights_path = "/playpen-raid2/lin.tian/projects/ByConstructionICON/results/oasis_train/network_weights_70000"
utils.log(net.regis_net.load_state_dict(torch.load(weights_path), strict=True))
net.eval()

for JJ in range(3):
    image_A = itk.imread(f"evaluation_results/oasis_eval_dump-2/pair{JJ}imageA.nii.gz")
    image_B = itk.imread(f"evaluation_results/oasis_eval_dump-2/pair{JJ}imageB.nii.gz")

    #import pdb; pdb.set_trace()
    phi_AB, phi_BA = itk_wrapper.register_pair(net, image_A, image_B, finetune_steps=None)

    
    render_pair(f"OASIS{JJ}", image_A, phi_AB, image_B, net)
